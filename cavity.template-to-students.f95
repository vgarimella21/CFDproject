...
module set_inputs ! Sets the input variables for the code
  ...
  Real(kind=Prec) ::      cfl  = 0.25_Prec       ! CFL number used to determine time step
  Real(kind=Prec) ::      Cx = 0.02_Prec        ! Parameter for 4th order artificial viscosity in x
  Real(kind=Prec) ::      Cy = 0.02_Prec        ! Parameter for 4th order artificial viscosity in y
  Real(kind=Prec) ::      toler = 1.e-4_Prec   ! Tolerance for iterative residual convergence
  Real(kind=Prec) ::      rkappa = 0.5_Prec     ! Time derivative preconditioning constant
  ...
  Real(kind=Prec) ::      fsmall = 1.e-6_Prec  ! small parameter
...
! Bottom wall: no-slip, stationary wall
  j = 1
  do i = 1, imax
    u(i,j,2) = zero   ! u-velocity
    u(i,j,3) = zero   ! v-velocity
  enddo

  ! Top wall: moving lid, u = uinf, v = 0
  j = jmax
  do i = 1, imax
    u(i,j,2) = uinf   ! moving lid
    u(i,j,3) = zero   ! no penetration
  enddo
  ...
  ! Bottom and top wall
  do i = 1, imax
    u(i,1,1) = u(i,2,1)           ! bottom - j = 1
    u(i,jmax,1) = u(i,jmax-1,1)   ! top - j = jmax
  enddo

  ! Left and right walls
  do j = 1, jmax
    u(1,j,1) = u(2,j,1)           ! i = 1
    u(imax,j,1) = u(imax-1,j,1)   ! i = imax
  enddo
...
!#######
  ...
  Real(kind=Prec) ::      hmin                     ! Minimum grid spacing


  ! constant viscous time step limit
  hmin = min(dx, dy)
  dtvisc = fourth * rho * hmin*hmin / rmu

  dtmin = 1.0e99_Prec  ! initialize global minimum
  ...
  do j = 2, jmax-1
    ...
    lambda_x = abs(u(i,j,2)) + sqrt(beta2)
    lambda_y = abs(u(i,j,3)) + sqrt(beta2)
...
!  use set_constants, only : zero, one, two, four, half, fourth
  ...
  use set_precision, only : Prec
  use set_constants, only : zero, four, six
  use set_inputs, only : imax, jmax, lim, dx, dy, Cx, Cy, &
       &                 vel2ref, rkappa
  ...
  Integer :: i, j
  Real(Prec) :: uvel2, beta2
  Real(Prec) :: lambda_x, lambda_y
  Real(Prec) :: d4pdx4, d4pdy4

  ! Zero out S contributions
  do j = 1, jmax
    do i = 1, imax
      artviscx(i,j) = zero
      artviscy(i,j) = zero
  ...
  ! Interior points (need 2 points away from boundaries for 4th derivative)
  do j = 3, jmax-2
    do i = 3, imax-2

      ! Local velocity magnitude squared
      uvel2 = u(i,j,2)*u(i,j,2) + u(i,j,3)*u(i,j,3)
      beta2 = max(uvel2, rkappa*vel2ref)
      ...
      ! 4th derivatives of pressure
      d4pdx4 = ( u(i-2,j,lim) - four*u(i-1,j,lim) + six*u(i,j,lim) &
           &     - four*u(i+1,j,lim) + u(i+2,j,lim) ) / (dx*dx*dx*dx)

      d4pdy4 = ( u(i,j-2,lim) - four*u(i,j-1,lim) + six*u(i,j,lim) &
           &     - four*u(i,j+1,lim) + u(i,j+2,lim) ) / (dy*dy*dy*dy)

      ! S_x and S_y contributions (eq. (4))
      if (beta2 > 0.0_Prec) then
        artviscx(i,j) = - ( (abs(lambda_x)**3) * Cx * (dx**3) / beta2 ) * d4pdx4
        artviscy(i,j) = - ( (abs(lambda_y)**3) * Cy * (dy**3) / beta2 ) * d4pdy4
      else
        artviscx(i,j) = zero
        artviscy(i,j) = zero
      endif
  ...
  use set_precision, only : Prec
  use set_constants, only : two
  use set_inputs, only : imax, jmax, rho, dx, dy, rkappa, rmu, vel2ref
  ...
  Real(Prec) :: d2udx2, d2udy2, d2vdx2, d2vdy2
  Real(Prec) :: beta2, uvel2
  Real(Prec) :: R0, R1, R2
  Real(Prec) :: D0, D1, D2
  Real(Prec) :: conv_u, conv_v
  Real(Prec) :: diff_u, diff_v
  Real(Prec) :: S_loc

  do i = 2, imax-1
    do j = 2, jmax-1

      ! Central differences using OLD values
      ...
      ! ---- artificial viscosity S only in continuity ----
      S_loc = artviscx(i,j) + artviscy(i,j)

      ! Continuity residual: dudx + dvdy = s_mass + S
      R0 = (dudx + dvdy) - s(i,j,1) - S_loc

      ! x-momentum residual
      conv_u = uold(i,j,2)*dudx + uold(i,j,3)*dudy
      diff_u = d2udx2 + d2udy2
      R1 = rho*conv_u + dpdx - rmu*diff_u - s(i,j,2)

      ! y-momentum residual
      conv_v = uold(i,j,2)*dvdx + uold(i,j,3)*dvdy
      diff_v = d2vdx2 + d2vdy2
      R2 = rho*conv_v + dpdy - rmu*diff_v - s(i,j,3)

      ! Approximate diagonals
      D0 = 1.0_Prec / dt(i,j)
      D1 = rho / dt(i,j) + two*rmu*(1.0_Prec/(dx*dx) + 1.0_Prec/(dy*dy))
      D2 = rho / dt(i,j) + two*rmu*(1.0_Prec/(dx*dx) + 1.0_Prec/(dy*dy))

      ! Jacobi update
      u(i,j,1) = uold(i,j,1) - (R0 / D0)
      u(i,j,2) = uold(i,j,2) - (R1 / D1)
      u(i,j,3) = uold(i,j,3) - (R2 / D2)
  ...
  do i = 2, imax-1
    do j = 2, jmax-1
      ...
      conv_u = u(i,j,2)*dudx + u(i,j,3)*dudy
      diff_u = d2udx2 + d2udy2
      R1 = rho*conv_u + dpdx - rmu*diff_u - s(i,j,2)

      conv_v = u(i,j,2)*dvdx + u(i,j,3)*dvdy
      diff_v = d2vdx2 + d2vdy2
      R2 = rho*conv_v + dpdy - rmu*diff_v - s(i,j,3)
      ...
      u(i,j,1) = u(i,j,1) - (R0 / D0)
      u(i,j,2) = u(i,j,2) - (R1 / D1)
      u(i,j,3) = u(i,j,3) - (R2 / D2)
  ...
  use set_inputs,     only : imax, jmax, neq, fsmall, dx, dy, rho, rmu
  use variables,      only : u, res, resinit, ninit, rtime, dtmin, s, &
                             artviscx, artviscy
  ...
  real(Prec) :: dudx, dudy
  real(Prec) :: dvdx, dvdy
  real(Prec) :: d2udx2, d2udy2
  real(Prec) :: d2vdx2, d2vdy2
  real(Prec) :: R0, R1, R2
  real(Prec) :: conv_u, conv_v
  real(Prec) :: diff_u, diff_v
  real(Prec) :: S_loc
  ...
  ! Only interior nodes
  num_nodes = real( (imax-2)*(jmax-2), kind=Prec )
  ...
  do i = 2, imax-1
    do j = 2, jmax-1
      ...
      ! Derivatives using CURRENT solution u
      dpdx = (u(i+1,j,1) - u(i-1,j,1)) / (two*dx)
      dpdy = (u(i,j+1,1) - u(i,j-1,1)) / (two*dy)
      ...
      ! x-momentum residual (no S-derivative terms in update)
      conv_u = u(i,j,2)*dudx + u(i,j,3)*dudy
      diff_u = d2udx2 + d2udy2
      R1 = rho*conv_u + dpdx - rmu*diff_u - s(i,j,2)
      ...
      res_sum(1) = res_sum(1) + R0*R0
      res_sum(2) = res_sum(2) + R1*R1
      res_sum(3) = res_sum(3) + R2*R2
  ...
  ! L2 norms (scaled)
  do k = 1, neq
    res_k = sqrt( res_sum(k) / max(num_nodes, 1.0_Prec) )
  ...
  Real(kind=Prec) ::      num_nodes                ! Number of nodes for averaging
  Real(kind=Prec) ::      u_exact                  ! Exact solution from MMS
  ...
  ! Initialize norms to zero
    ...
    ! Number of nodes (all interior and boundary points)
    num_nodes = real(imax * jmax, kind=Prec)
    ...
    do j = 1, jmax
      do i = 1, imax
        ...
        x = (xmax - xmin) * real(i - 1, kind=Prec) / real(imax - 1, kind=Prec)
        y = (ymax - ymin) * real(j - 1, kind=Prec) / real(jmax - 1, kind=Prec)
        
        ! Loop over each equation (pressure, u-velocity, v-velocity)
        do k = 1, neq
          
          ! Get exact solution
          u_exact = umms(x, y, k)
          
          ! Compute absolute discretization error
          DE = abs(u(i,j,k) - u_exact)
          
          ! Accumulate L1 norm (sum of absolute errors)
          rL1norm(k) = rL1norm(k) + DE
          
          ! Accumulate L2 norm (sum of squared errors)
          rL2norm(k) = rL2norm(k) + DE * DE
          
          ! Update Linf norm (maximum absolute error)
          if (DE > rLinfnorm(k)) then
            rLinfnorm(k) = DE
          endif
    ...
    ! Normalize L1 norm by number of nodes
    do k = 1, neq
      rL1norm(k) = rL1norm(k) / num_nodes
    enddo

    ! Normalize L2 norm (take square root and divide by number of nodes)
    do k = 1, neq
      rL2norm(k) = sqrt(rL2norm(k) / num_nodes)
    enddo
